...
def front_x(words):
    contador = 0
    arreAux = []
    #Busqueda palabras que empiezen con x
    for palabra in words:
        if (palabra[0] == 'x' or palabra[0] == 'X'):
            arreAux.append(palabra)
            
    #Elimina palabras con x en words        
    for palabra in arreAux:
    ...
    #Inserta primero las palabra con x ordenadas
    for palabra in arreAux: 
        words.insert(contador, palabra)
        contador = 1 + contador
...
def main():
    ...
    print
    print 'front_x'
    test(front_x(['bbb', 'ccc', 'xzz', 'xaa', 'axx']),
         ['xaa', 'xzz', 'axx', 'bbb', 'ccc'])
