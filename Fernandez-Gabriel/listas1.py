...
def match_ends(words):
    ...
    for cadena in words:
        palabra_1 = cadena[:1]
        palabra_2 = cadena[-1:]
        if len(cadena) >= 2 and palabra_1 == palabra_2:
            count += 1
    return count
...
def front_x(words):
    ...
    for cadena in words:
        ...
        if palabra == "x":
            ...
            lista_2.append(cadena)

    lista = sorted(lista_1) + sorted(lista_2)
...
def sort_last(tuples):
    # +++tu código aquí+++
    lista_ordenada = sorted (tuples, key=lambda tuples: tuples[-1:])
